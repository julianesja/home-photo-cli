...
class CallProcessPhoto:
    def process_photo(self, image: str | bytes)->tuple[bool, Photo, str]:
        file_content: bytes
        if isinstance(image, (str)):
            file_content = open(image, "rb").read()
        else:
            file_content = image
        config = Settings()
        ...
        result = process_photo.execute(file_content)
        return result
